...
// ─── Quality scoring ──────────────────────────────────────────────────
//
// Idiom-centric version: all xinhua entries are 成语, so tags are
// uniform. We boost classical-sounding entries (have 古文 explanation
// mentions) and penalize very rare or archaic ones.

const ARCHAIC_PENALTY_PATTERNS = [
  /鄙|贱|陋|妓|妾|奴/, // somewhat pejorative / archaic register
];
...
function scoreIdiom(entry) {
  ...
  // Length preference: 4-char gets full; 3/5/6 partial; longer less.
  if (n === 4) score += 0.1;
  else if (n === 3 || n === 5) score += 0.05;
  else if (n === 6) score += 0.02;
  else score -= 0.1;

  // Prefer entries with actual explanation/derivation (better-attested).
  if (entry.explanation && entry.explanation.length > 10) score += 0.05;
  if (entry.derivation && entry.derivation.length > 10) score += 0.03;

  // Slight penalty for archaic/obscure vocabulary.
  for (const re of ARCHAIC_PENALTY_PATTERNS) {
    if (re.test(text)) {
      score -= 0.08;
      break;
    }
